  ...
  return (
    <div className={`flex flex-col items-center min-h-screen bg-bodyBg-color text-heading-color ${i18n.language === 'ar' ? 'rtl' : 'ltr'}`}>
      <div className={`w-full ${i18n.language === 'ar' ? 'text-right' : 'text-left'} p-10 mt-20`}>
        <h1 className="text-3xl font-bold text-secondary-color">{t('Trader Details')}</h1>
      </div>
      ...
      {activeTab === t('Summary') && (
        <div className="w-full max-w-screen-lg px-4 sm:px-6 lg:px-8 text-black">
          <div className='bg-white p-4 rounded-xl'>
            <h3 className="text-lg font-bold mb-2">{t('Trader Information')}</h3>
            <div className="space-y-2">
              <div><strong>{t('Name')}:</strong> {trader.name}</div>
              <div><strong>{t('Type')}:</strong> {trader.type}</div>
              <div><strong>{t('Contact Info')}:</strong> {trader.contact_info}</div>
            </div>
          </div>
        </div>
      )}
