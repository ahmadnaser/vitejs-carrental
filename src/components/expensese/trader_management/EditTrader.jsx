...
import { updateTrader, getTraderById } from '../../../controller/TraderController';

const EditTraderForm = () => {
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
  const location = useLocation();
  const { traderId } = location.state || {};
  ...
  useEffect(() => {
    const fetchTraderData = async () => {
      try {
        const traderData = await getTraderById(traderId);
  ...
  const validateForm = () => {
    const errors = {};
    if (!formData.name.trim()) errors.name = t('Trader Name is required');
    if (!formData.type.trim()) errors.type = t('Trader Type is required');
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus('loading');
    try {
      const response = await updateTrader(traderId, formData);
      if (response.success) {
        setStatus('success');
  ...
  return (
    <div className={`flex flex-col items-center min-h-screen bg-bodyBg-color text-heading-color ${i18n.language === 'ar' ? 'rtl' : 'ltr'}`}>
      <div className={`w-full ${i18n.language === 'ar' ? 'text-right' : 'text-left'} p-10 mt-20 mb-10`}>
        ...
        <h3 className="font-bold text-l mt-3 text-heading-color cursor-pointer">{t('Edit Trader')}</h3>
      </div>

      <form onSubmit={handleSubmit} className={`w-full ${i18n.language === 'ar' ? 'text-right' : 'text-left'} p-10 mt-15 mb-10 max-w-md mx-auto`}>
        <div className="mb-5">
          <label htmlFor="name" className="block mb-2 text-sm font-medium">{t('Trader Name')}</label>
          <input type="text" name="name" value={formData.name} onChange={handleChange} className="rounded-lg text-gray-900 focus:outline-none focus:border-secondary-color focus:ring focus:ring-secondary-color focus:ring-opacity-100 text-sm block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder={t('Name of Trader')} required />
          {errors.name && <div className="text-red-500">{errors.name}</div>}
        </div>
        <div className="mb-5">
          <label htmlFor="type" className="block mb-2 text-sm font-medium">{t('Trader Type')}</label>
          <input type="text" name="type" value={formData.type} onChange={handleChange} className="rounded-lg text-gray-900 focus:outline-none focus:border-secondary-color focus:ring focus:ring-secondary-color focus:ring-opacity-100 text-sm block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder={t('Trader Type')} required />
