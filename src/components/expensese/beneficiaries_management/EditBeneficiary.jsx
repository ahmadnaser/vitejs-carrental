...
import { updateBeneficiary, getBeneficiaryById } from '../../../controller/BeneficiaryController'; 
...
const EditBeneficiaryForm = () => {
  ...
  useEffect(() => {
    const fetchBeneficiaryData = async () => {
      try {
        ...
        if (beneficiaryData) {
          setFormData({
            name: beneficiaryData[0].name,
            type: beneficiaryData[0].type,
            contact_info: beneficiaryData[0].contact_info,
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus('loading');
    try {
      const response = await updateBeneficiary(beneficiaryId, formData);
      if (response.success) {
        setStatus('success');
  ...
  return (
    <div className={`flex flex-col items-center min-h-screen bg-bodyBg-color text-heading-color ${i18n.language === 'ar' ? 'rtl' : 'ltr'}`}>
      <div className={`w-full ${i18n.language === 'ar' ? 'text-right' : 'text-left'} p-10 mt-20 mb-10`}>
        ...
        <h3 className="font-bold text-l mt-3 text-heading-color cursor-pointer">{t('Beneficiary Details')}</h3>
      </div>
      ...
      <form onSubmit={handleSubmit} className={`w-full ${i18n.language === 'ar' ? 'text-right' : 'text-left'} p-10 mt-15 mb-10 max-w-md mx-auto`}>
        ...
        {status === 'success' && (
          <div className="text-green-500 mt-4 flex items-center justify-center h-5 ">
            {t('Beneficiary updated successfully!')}
          </div>
        )}
        {status === 'error' && (
