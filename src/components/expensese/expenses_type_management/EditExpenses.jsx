...
import { updateExpensesType, getExpensesTypesById } from '../../../controller/ExpensesController';

const EditExpenseTypeForm = () => {
  ...
  useEffect(() => {
    const fetchExpenseTypeData = async () => {
      try {
        ...
        setFormData({
          type: expenseTypeData[0].type || '',
          type_info: expenseTypeData[0].type_info || '',
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus('loading');
    try {
      const response = await updateExpensesType(expensesTypeId, formData);
      if (response.success) {
        setStatus('success');
  ...
  return (
    <div className={`flex flex-col items-center min-h-screen bg-bodyBg-color text-heading-color ${i18n.language === 'ar' ? 'rtl' : 'ltr'}`}>
      <div className={`w-full ${i18n.language === 'ar' ? 'text-right' : 'text-left'} p-10 mt-20 mb-10`}>
        ...
        <h3 className="font-bold text-l mt-3 text-heading-color cursor-pointer">{t('Edit Expense Type')}</h3>
      </div>
      ...
      <form onSubmit={handleSubmit} className={`w-full ${i18n.language === 'ar' ? 'text-right' : 'text-left'} p-10 mt-15 mb-10 max-w-md mx-auto`}>
        ...
        {status === 'success' && (
          <div className="text-green-500 mt-4 flex items-center justify-center h-5 ">
            {t('Expense type updated successfully!')}
          </div>
        )}
        {status === 'error' && (
...
export default EditExpenseTypeForm;
