...
const GarageAccountStatementForm = () => {
  ...
  const handleNavigation = (path) => {
    ...
    if (!formData.garage_id.trim()) validationErrors.garage_id = t('Garage is required.');
  ...
  return (
    <div className={`flex flex-col items-center min-h-screen bg-bodyBg-color text-heading-color ${i18n.language === 'ar' ? 'rtl' : 'ltr'}`}>
      ...
      <form className={`w-full ${i18n.language === 'ar' ? 'text-right' : 'text-left'} p-10 mt-15 mb-10 max-w-md mx-auto`}>
        <div className="mb-5">
          <label htmlFor="garage" className="block mb-2 text-sm font-medium">{t('Garages')}</label>
          <div className="relative max-w-sm">
            <Select
              ...
              required
            />
            {errors.garage_id && <span className="text-red-500 mt-2 text-sm">{errors.garage_id}</span>}
          </div>
        </div>
        ...
        <div className="mb-5 mt-10 flex flex-col justify-center sm:flex-row sm:space-x-4 space-y-4 sm:space-y-0">
          ...
          <button
            type="button"
            onClick={() => handleNavigation('/reports/garage-statement/statement')}
...
export default GarageAccountStatementForm;
