...
const ReturnCarTable = () => {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    items = items.filter(item => item.has_return !== 1);
  ...
  return (
    <div className={`flex flex-col items-center min-h-screen bg-bodyBg-color text-heading-color ${i18n.language === 'ar' ? 'rtl' : 'ltr'}`}>
      ...
      {!rentedCarData && <div className='text-secondary-color'>Loading...</div>}
            ...
            {filteredItems.length === 0 ? (
              ...
              filteredItems.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-4 text-center">{item.has_return === 1 ? t('Yes') : t('No')}</td>


                  <td class="px-4 py-4 text-center">
                    <Link 
                      to="/return-car/return" 
