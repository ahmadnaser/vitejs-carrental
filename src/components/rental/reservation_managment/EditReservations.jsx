...
import { getContractsByRentalId, updateContract } from '../../../controller/RentedCarController';
...
const EditReservationForm = () => {
  ...
  const [isCarState, setIsCarState] = useState(true);  
  ...
  const fetchContractData = async () => {
    ...
    try {
        ...
        if (contract.length > 0) {
            ...
            const startDateTime = new Date(contract[0]?.start_date);
            ...
            setFormData({
                ...
                end_date: endDate,
                price_perday: contractData.price_perday || '',
                total_amount: contractData.total_amount || '',
                car_mileage: contractData.car_mileage || '',
                note: contractData.note || '',
                car_condition: contractData.car_condition || '',  
                car_damage: contractData.car_damage || '',      
            });

            setTime(startTime); 
            setReturnTime(returnTime);
            setSelectedTenant({ value: contractData.tenantID, label: `${contractData.customer} - ${contractData.tenantID}` });
            setSecondDriverId({ value: contractData.second_driver_id, label: `${contractData.second_driver_name} - ${contractData.second_driver_id}` });
            setSelectedCar({ value: contractData.vehicle_id, label: `${contractData.make} ${contractData.model} - ${contractData.vehicle_id}` });
            setIsCarState(!!(contractData.car_condition || contractData.car_damage)); 
...
const handleEndDateChange = (e) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await updateContract(rentalId, submissionData);
  ...
  return (
    <div className={`flex flex-col items-center min-h-screen bg-bodyBg-color text-heading-color ${i18n.language === 'ar' ? 'rtl' : 'ltr'}`}>
      <div className={`w-full ${i18n.language === 'ar' ? 'text-right' : 'text-left'} p-10 mt-20 mb-10`}>
        <h1 className="text-3xl font-bold text-secondary-color">{t('Edit Rental Contract')}</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className={`w-full ${i18n.language === 'ar' ? 'text-right' : 'text-left'} p-10 mt-15 mb-10 max-w-md mx-auto`}>
        ...
        {status === 'success' && (
          <div className="text-green-500">
            {t('Rental Contract Updated successfully! Redirecting...')}
          </div>
        )}
